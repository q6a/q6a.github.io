function debounce(func,wait,immediate){var timeout;return function(){var context=this,args=arguments;var later=function(){timeout=null;if(!immediate)func.apply(context,args);};var callNow=immediate&&!timeout;clearTimeout(timeout);timeout=setTimeout(later,wait);if(callNow)func.apply(context,args);};};"use strict";window.config=window.config||{};window.config.useLocalApp=false;$(document).ready(function(){var btnFTStart=$("#freeTrialButton");var btnFTLangConfirm=$("#langConfirm");var btnFTSubmit=$("#signupSubmit");var inputFTEmail=$("#emailSub");var inputFTLegalAccept=$("#legalinfoch");var appDisabled=btnFTStart.hasClass("appDisabled");var modalLanguages=$("#confirm-language");var modalLegal=$("#legalModal");function isEmail(str){var re=/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/;return!!str.match(re);}
function handleFTEmailChange(e){var value=e.target.value;if(!isEmail(value)){inputFTEmail.css("border","1px solid red");btnFTStart.attr("disabled","disabled");$(".tooltip-outer").attr("data-original-title","Disabled: Invalid Email Address").tooltip("fixTitle");}else{inputFTEmail.css("border","1px solid green");btnFTStart.removeAttr("disabled");$(".tooltip-outer").attr("data-original-title","Enabled: Valid Email Address").tooltip("fixTitle");}}
inputFTEmail.bind("change keyup",handleFTEmailChange);function handleFTStartClick(e){e.preventDefault();function next(){modalLanguages.modal("show");}
window.gtag?window.gtag("event","button",{event_category:"free-trial",event_label:"hero-next",event_callback:next}):next();}
btnFTStart.click(handleFTStartClick);function getSelectedSecondaryLanguages(){var inputsLanguages=modalLanguages.find("input[type=checkbox]:checked");var results=[];inputsLanguages.each(function(){if(this.value==="English"){return;}
...
modalLanguages.find("input").change(handleFTLangOptionChecked);function handleFTLangConfirmClick(e){function next(){modalLanguages.modal("hide");modalLegal.modal("show");}
window.gtag?window.gtag("event","button",{event_category:"free-trial",event_label:"languages-next",event_callback:next}):next();}
btnFTLangConfirm.click(handleFTLangConfirmClick);function handleFTLegalAcceptChange(e){if(this.checked){btnFTSubmit.removeAttr("disabled");}else{btnFTSubmit.attr("disabled","disabled");}}
...
btnFTSubmit.click(handleFTSubmitClick);});;"use strict";window.config=window.config||{};window.config.useGtagStub=false;function initNavbarPageScroll(){var $navbar=$("nav.navbar");var navbarStuckClass="navbar-stuck";function onScroll(){if($(this).scrollTop()>=40){$navbar.addClass(navbarStuckClass);}else{$navbar.removeClass(navbarStuckClass);}}
...
$(document).ready(function(){initNavbarPageScroll();initNavbarPageResize();initNavbarDropdownHover();initGATriggers();$('[data-toggle="tooltip"]').tooltip();});