...
function handleFTLangOptionChecked(e){var secondaryLanguages=getSelectedSecondaryLanguages();if(secondaryLanguages.length>=3){modalLanguages.find("input[type=checkbox]").each(function(){if(this.value==="English"||this.name=="legalinfo"){return;}
...
btnFTSubmit.click(handleFTSubmitClick);});$("#search-query").keypress(function(event){var keycode=event.keyCode?event.keyCode:event.which;if(keycode=="13"){handleBtnSearchPopup();}});function handleBtnSearchPopup(e){$('#search-results').empty();searchQuery=$('#search-query').val();if(searchQuery){gtag('event','search event',{'event_category':'search category','event_label':'search '+searchQuery,});$('#search-query').val(searchQuery);executeSearch(searchQuery);}else{$('#search-results').append('<p>Please enter a word or phrase above !</p>');}}
